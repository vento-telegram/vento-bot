...
async def set_mode_veo_video(
    ...
    text = (
        ...
        "⚠️ На данный момент нельзя генерировать видео в улучшенном качестве с добавленной картинкой. А также прикреплять изображения в формате 9:16\n\n"
...
async def set_mode_sora2_video(
    ...
    text = (
        ...
        "⏩ Когда настройки выбраны, просто отправь запрос с описанием нужного видео или сценарием, можешь прикрепить одну или несколько картинок.\n\n"
        "🔄 Если захочешь сменить режим или очистить контекст — используй команду /start"
    )
...
async def goto_referral(call: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        "2️⃣ Он переходит по ней и регистрируется в боте.\n"
        "3️⃣ Когда друг купит любой пакет токенов — тебе прилетит *+100 токенов* 🎉\n"
        "💡 Бонус начисляется только после первой покупки приглашённого пользователя.\n\n"
        "🔥 *Без ограничений!*\n"
